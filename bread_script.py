...
experiment_path = os.path.join(results_path, "bread")
...
#n_s = [9]
#s
executor_1 = submitit.AutoExecutor(folder="utils/truefinal/bread")
...
with executor_1.batch():
    for lr_2 in lr_2s:
        job = executor_1.submit(bread_exp, D = 900, teacher = w_teacher, student = student[1], T = T, lr_1 = 1, lr_2 = lr_2 , steps = 8000, experiment_path = run_path)
        jobs.append(job)
...
with executor_2.batch():
    for lr_2 in lr_2s:
        job_2 = executor_2.submit(n_or_more_neg, D = 900, teacher = w_teacher, student = student[1], T = T, lr_1 = 1, lr_2 = lr_2, steps = 8000, experiment_path = run_path)
        jobs_2.append(job_2)
